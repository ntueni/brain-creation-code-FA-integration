...
# --- CONFIGURATION ---
base_path = "./output_atrophy/post_processing/rampp_FA_9R/"
fileNames = [
    "rampp_atrophy_9R_last_timestep",
    "rampp_atrophy_17R_last_timestep",
    "rampp_atrophy_FA_last_timestep"
]
output_dir = os.path.join(base_path, "boxplots_stats_three_models")
...
def write_tex_figure_for_metric(df_all: pd.DataFrame, metric: str):
    """Emit a LaTeX figure with Cohen's d labels and outlier-filtered y-range - THREE models."""
    df = df_all[df_all['Metric'] == metric].copy()
    if df.empty:
        print(f"[WARN] Skipping TeX for {metric}: no data.")
        return

    # Map models to Sim1/Sim2/Sim3 by input order
    if len(model_names) < 3:
        print(f"[WARN] Skipping TeX for {metric}: need three models.")
        return
    model_to_sim = {model_names[0]: 'Sim1', model_names[1]: 'Sim2', model_names[2]: 'Sim3'}
    ...
    region_ymax = {} # region -> max across all three sims (for label placement)
    ...
    # Compute Cohen's d per region - comparing Sim1 vs Sim3 (9R vs FA)
    def compute_local_d(region_name):
        sim1 = df[(df['Region'] == region_name) & (df['sim'] == 'Sim1')]['Value'].to_numpy()
        sim3 = df[(df['Region'] == region_name) & (df['sim'] == 'Sim3')]['Value'].to_numpy()
        if sim1.size == 0 or sim3.size == 0:
            return np.nan
        n = min(sim1.size, sim3.size)
        sim1 = sim1[:n]
        sim3 = sim3[:n]
        diffs = sim1 - sim3
    ...
    base_positions = [i*4 for i in range(len(region_order))]  # 0,4,8,... (increased spacing for 3 bars)
    xticks = [b+1.5 for b in base_positions]                   # 1.5,5.5,9.5,...
    ...
    with open(tex_path, 'w') as f:
        f.write("% Auto-generated by Python: boxplots with Cohen's d labels (outlier-filtered y-range) - THREE MODELS\n")
        f.write("%%%%%%%%%%%%%%%%%%%%%%%%\n")
        ...
        f.write("      xmin=0.5, xmax=%s,\n" % tex_sanitize_float(base_positions[-1] + 3.5))
        ...
        # Legend entries - THREE models
        f.write("      % manual legend entries\n")
        f.write("      \\addlegendimage{area legend, draw=OI-Black, fill=OI-Blue, opacity=0.8}\n")
        f.write("      \\addlegendentry{boxplot 9R region based $\\mu$}\n\n")
        f.write("      \\addlegendimage{area legend, draw=OI-Black, fill=OI-Green, opacity=0.8}\n")
        f.write("      \\addlegendentry{boxplot 17R region based $\\mu$}\n\n")
        f.write("      \\addlegendimage{area legend, draw=OI-Black, fill=OI-Yellow, opacity=1}\n")
        f.write("      \\addlegendentry{boxplot FA voxel based $\\mu$}\n\n")
        ...
        # Emit plots for each region - THREE models
        for idx, region in enumerate(region_order):
            ...
            for sim, pos, fill, opacity in [('Sim1', pos_sim1, 'OI-Blue', '0.8'), 
                                             ('Sim2', pos_sim2, 'OI-Green', '0.8'),
                                             ('Sim3', pos_sim3, 'OI-Yellow', '1.0')]:
                summ = summaries.get((region, sim))
                if summ is None:
                    continue
                
                # Find top whisker across all three sims
                uw1 = summaries.get((region, 'Sim1'))
                uw2 = summaries.get((region, 'Sim2'))
                uw3 = summaries.get((region, 'Sim3'))
                top_whisker = -np.inf
                if uw1 is not None:
                ...
                if uw3 is not None:
                    top_whisker = max(top_whisker, uw3['upper_whisker'])
                ...
                f.write("        },\n")
                f.write(f"        fill={fill}, fill opacity={opacity},\n")
            ...
            # Cohen's d label (centered at tick position base+1.5) - comparing Sim1 vs Sim3
            ...
            y_label_offset = max(0.08 * y_range_for_label, abs(ylab) * 0.1)
            y_label = ylab + y_label_offset
            x_label = base + 1.5

            # Find top whisker for this region
            uw1 = summaries.get((region, 'Sim1'))
            uw2 = summaries.get((region, 'Sim2'))
            uw3 = summaries.get((region, 'Sim3'))
            top_whisker = -np.inf
        ...
        f.write(f"  \\caption{{Boxplots illustrating '{metric}' across nine brain regions for three atrophy models (9R, 17R, FA). The lower whisker is the 5\\textsuperscript{{th}} percentile and the upper whisker the 95\\textsuperscript{{th}} percentile. The arithmetic mean is marked. Cohen's $d$ (comparing 9R vs FA) is shown above each region.}}\n")
...
def export_metric_files(df_all: pd.DataFrame, metric: str):
    """Export CSV files for each metric - THREE models."""
    df = df_all[df_all['Metric'] == metric].copy()
    if df.empty:
    ...
    # Map models to Sim1/Sim2/Sim3 by input order
    if len(model_names) < 3:
        raise RuntimeError("Need three models to create comparisons.")
    model_to_sim = {model_names[0]: 'Sim1', model_names[1]: 'Sim2', model_names[2]: 'Sim3'}
    df['sim'] = df['Atrophy model'].map(model_to_sim)
    ...
    # stats csv: x/y + label text (p and d) - comparing Sim1 vs Sim3 (9R vs FA)
    rows = []
    for i, r in enumerate(region_order):
        g1 = df[(df['Region'] == r) & (df['sim'] == 'Sim1')]['Value'].to_numpy()
        g3 = df[(df['Region'] == r) & (df['sim'] == 'Sim3')]['Value'].to_numpy()

        if len(g1) == 0 or len(g3) == 0:
            p = np.nan
            d = np.nan
            ymax = np.nan
        else:
            _, p, d = compute_wilcoxon_cohens_d(g1, g3)
            # Get max across all three models for this region
            g2 = df[(df['Region'] == r) & (df['sim'] == 'Sim2')]['Value'].to_numpy()
            ymax = max(np.max(g1), np.max(g2) if len(g2) > 0 else -np.inf, np.max(g3))

        base = i*4 + 0.5
        x_label = base + 1.0  # centers at ticks: 1.5,5.5,9.5,...
        if np.isfinite(ymax):
...
print("TeX files now use outlier-filtered y-range for better visualization with THREE models.")